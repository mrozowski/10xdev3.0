...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "All three compound: coordination overhead + workflow friction + data trapped in chat"
    - topic: "primary persona scope"
      decision: "Both recurring friend groups AND one-off event groups (strangers coordinating a single event)"
  frs_drafted: 15
...
### Primary
- A host can create an event, add items, and share link the and a participant can open it, enter a nickname, and claim an item — all within 60 seconds total. This flow completing reliably = the product works. 
...
## Socrates Challenge Round (Phase 4.5)
...
FR-011 update (IMPORTANT — business rule change):
> Socrates: User clarified: if an item has quantity > 1, multiple participants can claim portions of that item until the full quantity is covered. FR-011 updated to reflect this. This is a core domain rule — see Business Logic.
...
Supporting detail:
- A claim records: participant nickname, item ID, and (for quantity items) the number of units claimed.
